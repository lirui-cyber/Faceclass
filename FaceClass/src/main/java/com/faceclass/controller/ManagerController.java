...
import com.faceclass.dto.CourseMessageDto;
import com.faceclass.entity.*;
import com.faceclass.service.*;
...
public class ManagerController {  
    	...
    	if( manager != null ){	//如果不等于空说明账号正确
    		if( manager.getManagerPassword().equals(password) ){
		    	...
		    	if( userDateNumberList != null && userDateNumberList.size() > 0 ){
		    		...
		    		// 将list转成json用于页面js操作
					String userDateNumberListStr = JSON.toJSONString(userDateNumberList);	//将java对象转换为json对象
		    						//将json对象转换为字符串
    ...
    public String managerCourseUpdateChapter(HttpServletRequest request, HttpSession session) throws IllegalStateException, IOException{
    	...
    	// 否则，如果小节id不为空，则更新小节信息
			...
			String oldVideoPath = "D:\\faceclass\\video\\" + oldVideoName;	// (旧的)文件上传的路径+文件名
			...
			if( multipartResolver.isMultipart(request) ){//获取文件解析器，判断是否是文件
				...
				while( iter.hasNext() ){//遍历所有文件
					...
					if( file != null && file.getOriginalFilename() != null && file.getOriginalFilename() != "" ){
						...
						String path = "D:\\faceclass\\video\\" + fileName;		//提交路径(用于保存文件到磁盘)；		数据库中只存放文件名，页面显示通过 虚拟路径配置
						
						// 判断D盘是否有 faceclass 文件夹，接着判断imooc文件夹下是否有video文件夹，没有就新建这两个文件夹，用于存储视频文件
						File folder = new File("D:\\faceclass");
						if( !folder.exists() && folder.isDirectory() ){
							folder.mkdir();		// 创建文件夹D:\\faceclass
						}
						File folder2 = new File("D:\\faceclass\\video");
						if( !folder2.exists() && folder2.isDirectory() ){
							folder2.mkdirs();	// 创建文件夹D:\\faceclass\\video
						}
    		...
    		if( deleteResult == 1 ){// 删除章成功
    			...
    			if( del != 0 ){// 删除所有的小节成功
    				...
    				for( SmallChapter smallChapter: smallChapterList ){
        	    		...
        	    		String videoPath = "D:\\faceclass\\video\\" + videoName;	// 文件上传的路径+文件名
    ...
    public String managerGetSortsOneByCode(@RequestParam("code") String code, HttpServletRequest request, HttpSession session){
    	if( code != null ){
    		...
    		if( sortsOne != null ){
				String str =  JSON.toJSONString(sortsOne);//将java对象转换为json对象
        		//将json对象转换为字符串
        		System.out.println("json" + str);
        		return str;
    		}else{
    ...
    public String managerGetSortsTwoByCode(@RequestParam("code") String code, HttpServletRequest request, HttpSession session){
    	if( code != null ){
    		...
    		if( sortsTwo != null ){
				String str =  JSON.toJSONString(sortsTwo);//将java对象转换为json对象
        		//将json对象转换为字符串
        		System.out.println("json" + str);
        		return str;
    		}else{
    ...
    public String managerGetUserById(User u, HttpServletRequest request, HttpSession session){
    	if( u.getUserId() != null ){
			...
			String str = JSON.toJSONString(user);//将java对象转换为json对象
    		//将json对象转换为字符串
